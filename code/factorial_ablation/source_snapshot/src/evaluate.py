...
def choose_accels_batched(
    ...
    """Choose learned-planner commands with one forward call across all robots.

    Candidate scoring and RVO filtering remain per-robot and therefore follow
    ``choose_accel`` exactly. Only the independent MLP queries are concatenated
    from ``n`` batches of five rows into one ``5n``-row call.
    """

    if method not in {"icgp_rvo_mpc", "passive_rvo_mpc", "residual_passive_rvo_mpc"}:
        commands = []
        latencies = []
        stats = []
        for ego, state in enumerate(states):
            command, latency, decision_stats = choose_accel(
                method,
                state,
                ego,
                cfg,
            ...
            stats.append(decision_stats)
        return np.asarray(commands, dtype=np.float32), latencies, stats

    predictor = "intent"
    if method == "passive_rvo_mpc":
        predictor = "passive"
    elif method == "residual_passive_rvo_mpc":
        predictor = "residual_passive"
    model = {
        ...
        "residual_passive": residual_passive_model if residual_passive_model is not None else intent_model,
    }[predictor]
    include_intent = predictor == "intent"
    action_indices = PLANNER_ACTION_INDICES
    features = np.stack(
        [
            make_feature(state, ego, action_index, cfg, include_intent=include_intent)
            for ego, state in enumerate(states)
            for action_index in action_indices
        ],
        axis=0,
    ...
    start = time.perf_counter()
    with torch.no_grad():
        raw_predictions = model(torch.from_numpy(features)).cpu().numpy()
    total_inference_ms = (time.perf_counter() - start) * 1000.0
    ...
    for ego, state in enumerate(states):
        ...
        neighbors = nearest_neighbors(state.pos, ego, cfg.neighbor_limit)
        candidate_rows = []
        for action_row, action_index in enumerate(action_indices):
            acc = action_to_accel(action_index, state, ego, cfg)
            ego_traj = ego_rollout(state, ego, acc, cfg, cfg.horizon)
            neighbor_traj = raw_predictions[ego, action_row, :, : len(neighbors), :] + state.pos[ego]
            predicted_margin = candidate_safety_margin(state, ego, ego_traj, neighbor_traj, cfg)
            runtime_margin = runtime_safety_margin(state, ego, acc, cfg)
            margin = min(predicted_margin, runtime_margin)
            cost = planner_cost(ego, state, ego_traj, neighbor_traj, acc, cfg)
        ...
        if not selection_rows:
            ...
            selection_rows = [row for row in candidate_rows if row[3] >= best_margin - 1.0e-6]
            stats["screen_all_rejected"] = 1.0
        for _action_index, acc, cost, margin, predicted_margin, runtime_margin in selection_rows:
            if cost < best_cost:
                best_cost = cost
                best_acc = acc
                stats["screen_selected_margin"] = float(margin)
        ...
        filtered = rvo_safety_filter(ego, best_acc, state, cfg)
        stats["rvo_intervened"] = float(np.linalg.norm(filtered - best_acc) > 1.0e-5)
        commands.append(filtered)
        latencies.append(total_inference_ms / len(states))
        stats_rows.append(stats)
    return np.asarray(commands, dtype=np.float32), latencies, stats_rows
...
def run_episode(
    ...
    batch_across_robots: bool = False,
) -> tuple[dict, pd.DataFrame]:
    scenario = make_scenario(scenario_name, n, seed, tier=tier, area_policy=area_policy)
    state = initial_state(scenario)
    noise_schedule = make_noise_schedule(scenario_name, n, seed, noise_level, cfg.max_steps)
    ...
    for step in range(cfg.max_steps):
        ...
        if batch_across_robots:
            commands, latencies, decision_stats_rows = choose_accels_batched(
                method,
                observed_states,
                cfg,
                intent_model,
            ...
            latencies = []
            decision_stats_rows = []
            for i, observed_state in enumerate(observed_states):
                command, latency, decision_stats = choose_accel(
                    method,
                    observed_state,
                    i,
                ...
                commands.append(command)
                latencies.append(latency)
                decision_stats_rows.append(decision_stats)
        for i, (observed_state, command, latency, decision_stats) in enumerate(
            zip(observed_states, commands, latencies, decision_stats_rows)
        ):
            acc[i] = command
    ...
    result = {
        ...
        "sigma_p": float(noise_schedule.level.sigma_p),
        "sigma_v": float(noise_schedule.level.sigma_v),
        "observation_noise_protocol": OBSERVATION_NOISE_PROTOCOL_VERSION,
