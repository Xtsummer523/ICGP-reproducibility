...
Large-language-model tools were used for manuscript linguistic polishing, structural organization, and auxiliary drafting of the Fig.~\ref{fig:concept} schematic. Tian Xia manually edited and adjusted the schematic and is responsible for its scientific accuracy and final content. All theoretical derivations, simulation data, statistical analyses, and figure content were reviewed, edited, and verified by Tian Xia. No AI tool qualifies as an author.
...
Each estimate links to paired records, derived summaries, evaluation code, model weights, and integrity checks. The archive contains 3,840 Gaussian records, 5,760 temporal-stress records, 1,440 prediction-diagnostic rows, and the 3,840-record factorial corpus. These materials support internal recomputation. Public release still requires a persistent access route, reuse licence, and verified redistribution permissions.
...
\textbf{Data availability.} The supplementary package contains the formal results, stress-test records, prediction diagnostics, factorial ablation, and ten-seed Gazebo archive. The records, manifests, and integrity checks support internal recomputation; a persistent repository record, access route, and reuse licence must be added before submission.

\textbf{Code and materials availability.} The revision package includes the simulator, evaluation and statistical code, figure sources, and predictor weights with integrity hashes. Redistribution remains subject to licence verification.
